...
function create_location($service) {
    $connection = connect();
    $name = $service["name"];
	$address = $service["address"];
    $vaccines = $service["services_available"];
    $query = "INSERT INTO dbLocations (name, address) VALUES ('$name', '$address')";
    $result = mysqli_query($connection, $query);
    if (!$result) {
        return null;
    }
    $id = mysqli_insert_id($connection);
    add_services_to_location($id, $vaccines);
    mysqli_commit($connection);
    mysqli_close($connection);
    return $id;
}

function add_services_to_location($locationID, $serviceIDs) {
    $connection = connect();
    foreach($serviceIDs as $serviceID) {
        $query = "insert into dbLocationsServices (locationID, serviceID) values ('$locationID', '$serviceID')";
        $result = mysqli_query($connection, $query);
        if (!$result) {
            return null;
        }
...
function find_location($id) {
    $connection = connect();
    $id = mysqli_real_escape_string($connection, $id);
    $query = "select * from dbLocations where id = '$id'";
    $result = mysqli_query($connection, $query);
...
function delete_location($id) {
    $query = "delete from dbLocations where id='$id'";
    $connection = connect();
    $result = mysqli_query($connection, $query);
    $result = boolval($result);
    mysqli_close($connection);
