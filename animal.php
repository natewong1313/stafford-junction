...
<body>
    ...
    <main class="event-info">
        ...
        <div id="dashboard">
            <?php
                ...
                foreach ($animal_events as $event) {
                    ...
                    if ($event["completed"] != "yes" && $event['date'] >= $current_date) {
        ...
        <p></p>
        <h2 class="centered">Incomplete Appointments - Please Complete</h2>
        <div id="dashboard">
            <?php
                $no_incomplete_appointments = true;
                foreach ($animal_events as $event) {
                    $event_date = date('F j, Y', strtotime($event['date']));
                    if ($event['completed'] != "yes" && $event['date'] < $current_date) {
                        echo '<div class="dashboard-item" data-link="event.php?id=' . $event['id'] . '">';
                        ...
                        echo '<span>' . $event_date . '</span>';
                        echo '</div>';
                        $no_incomplete_appointments = false;
                    }
                }
                if ($no_incomplete_appointments) {
                    echo '<div id="table-wrapper" class="centered"><p>There are no incomplete appointments.</p></div>';
                }
            ?>
        </div>
        <p></p>
        <h2 class="centered">Completed Appointments</h2>
        <div id="dashboard">
            <?php
                $no_complete_appointments = true;
                foreach ($animal_events as $event) {
                    $event_date = date('F j, Y', strtotime($event['date']));
                    if ($event['completed'] == "yes") {
                        ...
                        echo '<span>' . $event_date . '</span>';
                        echo '</div>';
                        $no_complete_appointments = false;
                    }
                }
                if ($no_complete_appointments) {
                    echo '<div id="table-wrapper" class="centered"><p>There are no completed appointments.</p></div>';
                }
            ?>
        </div>
